...
namespace pingu
{
    public partial class Seguidores : Form
    {
        ...
        private async void Seguidores_Load(object sender, EventArgs e)
        {
            InicializarPanelUsuarios();
            btnNuevoPost.BringToFront();
            await CargarSeguidoresAsync();
        }
        ...
        private string ObtenerIdSeguidor(Seguidor seg)
        {
            if (seg == null) return "";

            string valor;

            valor = JsonElementAString(seg.idSeguidor);
            if (!string.IsNullOrWhiteSpace(valor)) return valor;

            valor = JsonElementAString(seg.id_seguidor);
            if (!string.IsNullOrWhiteSpace(valor)) return valor;
        ...
        private async Task CargarSeguidoresAsync()
        {
            try
            {
                ...
                HashSet<string> idsSeguidores = new HashSet<string>(
                    seguidores
                        .Select(s => ObtenerIdSeguidor(s))
                        .Where(id => !string.IsNullOrWhiteSpace(id))
                );

                List<User> usuariosSeguidores = usuarios
                    .Where(u =>
                    {
                        string idUsuario = ObtenerIdUsuario(u);
                        return !string.IsNullOrWhiteSpace(idUsuario) && idsSeguidores.Contains(idUsuario);
                    })
                    .ToList();

                if (usuariosSeguidores.Count == 0)
                {
                    Label lblVacio = new Label();
                    lblVacio.Text = "Todavía no tienes seguidores.";
                ...
                foreach (User usuario in usuariosSeguidores)
                {
        ...
        private async void btnSeguidores_Click(object sender, EventArgs e)
        {
            await CargarSeguidoresAsync();
        }
        ...
        private void Seguidores_FormClosed(object sender, FormClosedEventArgs e)
        {
            if (cerrandoSesion)
            {
                this.DialogResult = DialogResult.Cancel;
