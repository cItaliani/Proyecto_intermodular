...
async function cargarFollows() {
    const contenedor = document.getElementById("follows");
    const idUsuarioLogueado = localStorage.getItem("id_usuario");

    contenedor.innerHTML = "<p>Cargando seguidos...</p>";

    try {
        const [seguidos, usuarios] = await Promise.all([
            obtenerFollows(idUsuarioLogueado),
            obtenerTodosLosUsuarios()
        ]);

        const idsSeguidos = seguidos.map(seg => String(obtenerIdSeguido(seg)));

        const usuariosSeguidos = usuarios.filter(usuario => {
            const idUsuario = String(obtenerIdUsuario(usuario));
            return idsSeguidos.includes(idUsuario);
        });

        if (!usuariosSeguidos.length) {
            contenedor.innerHTML = "<p>No sigues a ningún usuario.</p>";
            return;
        }

        const html = usuariosSeguidos.map(usuario => `
            <div class="user-card">
        ...
        console.error(error);
        contenedor.innerHTML = "<p>Error cargando seguidos.</p>";
    }
}
